...
public class lex {
   ...
   private token getStringAndIdentifier() {
      ...
      while(!inputProgram.isEOF()) {
         switch(state) {
            ...
            case 5: {
               int attribCounter = symbolTable.getAttribCounter(theString);
               if(attribCounter > 0) {
                  inputProgram.retract(); return new token(TOKEN_NAME.ID,theString,attribCounter); 
               } else {
                  inputProgram.retract(); return new token(TOKEN_NAME.ID,theString,"No Attrib"); 
               }
            }
                  ...
                  if(ct != null) {
                     if(ct.tokenName == TOKEN_NAME.ID) { //identifying keywords from identifiers...
                        if(ct.lexeme.equals("int") || ct.lexeme.equals("int ")) {ct.attribValue = "0"; ct.tokenName = TOKEN_NAME.INT; }
                           ...
                           if(!symbolTable.isAlreadyThere(ct.lexeme)) { //if we have the identifier already in the symbol table do not add it again
   ...
   if(invalidTokens.size() != 0) {
      System.out.println("");
      for(token t : invalidTokens) System.out.println("Lexical Error" +" : Unknown Lexeme at " +"line " + t.lineNumber  + " -> "  + t.lexeme);
   } else {
      ...
      //Print tokens if founded...
      {
         if(tokens.size() != 0) {
            ...
            System.out.println("No tokens here :(");
         }
      }
      ...
      if(singlelineComments.size() != 0) {
         ...
         for(token t : singlelineComments) System.out.println("Found Comment at " +"line " + t.lineNumber + " : " + t.lexeme);
      }     

      //Print all comments if founded...
      if(multilineComments.size() != 0) {
         System.out.println("\n"  + "=== MULTI LINE COMMENTS FOUNDED ===\n" );
         for(token t : multilineComments) System.out.println("Found Comment at "  +"line " + t.lineNumber  + " : " + t.lexeme);
      }
         symbolTable.print();
         //System.out.println(utils.ANSI_GREEN + "\nLexical Analysis Successful !" + utils.ANSI_RESET);
      }
   }
}