...
public class parser {
   ...
   public parser(ArrayList<token> tokens) {
      ...
      parseTable.put(new Item("0","id","s5")); //state, input, action/goto
   ...
   public boolean compile() {
      ...
      while(true) {
         ...
         if(data == null) {
            ...
            //System.out.print( + "Compilation Failed : " + utils.ANSI_RESET);
            if(input.equals("$")) { System.out.println("Syntax Error" +" at "+ "line 0" + " : Unexpected End Of Input");  }
            else {System.out.println("Syntax Error"  +" at " + "line 0" +  " : Unexpected symbol found -> "  +lexSource.getCurrentTokenLexeme() ); }
            return false; 
         }
            ...
            char ruleNumber = data.charAt(1); //Assuming we have number of rules not exceeding double digits...
            int popCap = getLHSL(Character.getNumericValue(ruleNumber)) * 2;

            for(int i = 1; i <= popCap; i++) { inputStack.pop(); } //Pop from stack according to Length of L.H.S...
            inputStack.push(getRHS(Character.getNumericValue(ruleNumber))); //Now get the R.H.S From the Rule And Push to stack...
            ...
            System.out.println(stepsCounter + ". REDUCE " + data  + " " + inputStack + " --> "  + lexSource.getCurrentToken()  + " -->"  + "[" + productionRules.get(Character.getNumericValue(ruleNumber-1)) + "]" );
