...
public class symboltable {
   ...
   public boolean isAlreadyThere(String lexeme) {
      for(token t : table) {
         if(t.lexeme.equals(lexeme)) return true;
      } return false;
   }

   public int getAttribCounter(String lexeme) {
       for(token t : table) {
         if(t.lexeme.equals(lexeme)) return t.attribCounter;
      } return -1;
   }
   ...
   public void print() {
      System.out.println("\n"  + "====== SYMBOL TABLE ======\n");
      for(token t : table) {
         System.out.println(t.attribCounter + "\t" + t.tokenName.toString().toLowerCase() + "\t" + "-" + "\t" + t.lexeme);
      }
   }
}